...
# prediction horizon
Np = 5
# system parameters p = 0 q =0.2 v =0.2 w = 0.2
A = np.array([[1,0],[0.004, 1]])
B = np.array([[0, 0.1],[0.1, 0]])
# terminal matrix
P = np.array([[13.6162,	-0.2232],[-2.2489,11.3463]])
K = np.array([[0.5123, -4.7250],[-5.4389,	0.2620]])

# system parameters p = -0.3 q =0.2 v =0.2 w = 0.2
# A = np.array([[0.9925,0.1411],[0.004, 1.0029]])
# B = np.array([[-0.15, 0.109],[0.1, -0.006]])
# # terminal matrix
# P = np.array([[12.3,	-3.9598],[4.0577,19.1053]])
# K = np.array([[1.5345, -4.5711],[-4.5076,	5.4590]])
...
desire_input = np.array([0.2,0.18])
desire_state = np.array([0,0.2])
desire_input_seq = np.array([desire_input[0],desire_input[1],desire_input[0],desire_input[1],desire_input[0],desire_input[1],
                           desire_input[0],desire_input[1],desire_input[0],desire_input[1]])

# control vector init
error = 0.05*np.ones(10)
x_desire_state = np.array([desire_state[0],desire_state[1],desire_state[0],desire_state[1],desire_state[0],desire_state[1],
                           desire_state[0],desire_state[1],desire_state[0],desire_state[1]])
x_ub = np.array([0.52, 0.39, 0.52, 0.39, 0.52, 0.39, 0.52, 0.39, 0.52, 0.39]) - error - x_desire_state
x_lb = np.array([-0.52, -0.39, -0.52, -0.39, -0.52, -0.39, -0.52, -0.39, -0.52, -0.39]) + error - x_desire_state
...
#        constant = 0.5
    ...
    # vel_msg = String()
    # vel_msg = 'It is '+str(input) + 'what is ' + str(icount)
