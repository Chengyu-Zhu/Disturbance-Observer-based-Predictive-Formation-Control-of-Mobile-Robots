...
class dualmode_mpc:
    ...
    def constraint_init(self,desire_x,desire_y,desire_phi):
        ...
        x_desire_state = np.array([desire_state[0],desire_state[1],desire_state[2],
                                ...
                                desire_state[0],desire_state[1],desire_state[2],
                                desire_state[0],desire_state[1],desire_state[2]])
        self.x_ub = 6*np.ones(self.Np*np.size(self.Phi,1)) - error - x_desire_state
        self.x_lb = -6*np.ones(self.Np*np.size(self.Phi,1)) + error - x_desire_state
        self.u_ub = 0.4*np.ones(self.Np*np.size(self.B,1)) - desire_input_seq
        self.u_lb = -0.4*np.ones(self.Np*np.size(self.B,1)) - desire_input_seq
    ...
    def update_ref(self,tao):
        ...
        difx1_tao = 0.01
        difx2_tao = 0 
        dify1_tao = 2*0.01*np.cos(0.01*tao)
        dify2_tao = -2*0.01*0.01*np.sin(0.01*tao)
    ...
    def set_opt(self):
        ...
        self.opti.minimize( c_opt.T@self.Psit@c_opt + self.taox1*self.W*self.taox1 + self.taox2*self.W*self.taox2 +self.taox3*self.W*self.taox3 +self.taox4*self.W*self.taox4)
